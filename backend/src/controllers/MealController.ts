...
export const registerMeal = async (req: Request, res: Response) => {
  try {
    const prisma = req.app.get('prisma');
    const { participantId, mealOptionId, date } = req.body;
    if (!participantId || !mealOptionId) return res.status(400).json({ message: 'participantId e mealOptionId são obrigatórios' });
    ...
    if (existing) return res.status(409).json({ message: 'Refeição já registrada para esta participante neste dia' });
...
export const getMeals = async (req: Request, res: Response) => {
  try {
    ...
    if (participantId) {
      const where: any = { participantId: Number(participantId) };
      if (mealOptionId) where.mealOptionId = Number(mealOptionId);
    ...
    const groups: Record<string, { date: string; type: string; count: number }> = {};
    raw.forEach((m: any) => {
      const d = m.date ? new Date(m.date).toISOString().slice(0,10) : 'unknown';
      const type = m.mealOption?.name || 'Desconhecido';
      const key = `${d}::${type}`;
      if (!groups[key]) groups[key] = { date: d, type, count: 0 };
      groups[key].count += 1;
    });
